...
List::List(const List& p)//конструктор копирования
{
...
List& List::operator=(const List& p)//оператор присваивания копирования
{
	if (this == &p) {
		return *this;
	}
...
List& List::operator=(List&& p)//оператор присваивания перемещения
{
	if (this == &p) {
		return *this;
	}
...
void List::push_back(int data)//добавление элемента без приоритета(автоматически дается low приоритет и ставится в конец очереди)
{
	if (head == nullptr)
	{
		head = new Node(data);
...
ostream& operator<<(ostream& s, const List& p)//вывод очереди в поток
{
	if (p.Size > 0) {
		int i = 1;
		List::Node* node = p.head;
